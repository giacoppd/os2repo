...
//		memcpy(dev->data + offset, buffer, nbytes);	
		for(n = 0; n < nbytes; n+= crypto_cipher_blocksize(cipher))
			crypto_cipher_encrypt_one(cipher,dev->data + offset + n, buffer + n);				printk("Encrypted\n");	
	}
	else{
//		memcpy(buffer, dev->data + offset, nbytes);
		for(n = 0; n < nbytes; n+= crypto_cipher_blocksize(cipher))
			crypto_cipher_encrypt_one(cipher, buffer + n, dev->data +  + offset + n);	
		printk("Decrypted\n");
	}
}	
